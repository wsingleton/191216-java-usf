/*
    In a SELECT statement, you ned to specify at least:
    
        - the columns taht you want to retrieve (column list)
        - the table where theose values are stored (FROM clause)
*/
...
-- Specify cetrain columns for retrieval
...
-- the ASC keyword is not requred when sorting in ascending order
SELECT *
FROM employees
ORDER BY birthdate;
...
-- we can use the column positions instead of their name (not recommended)
SELECT * 
FROM employees
ORDER BY 6 DESC, 5;
...
/*
    The DUAL table is used with SELECT statements when attemptin to
...
ADD altername_name VARCHAR2(25);

UPDATE products
SET altername_name = SUBSTR(prod_name, 1, 3)
WHERE prod_id < 4;

-- COUNT provides a count of the number of NON-NULL values in a column
SELECT COUNT(prod_id), COUNT(altername_name)
FROM products;
...
/*
    Retrieve all empmloyees and group them by their department and then
...
/*
    HAVING clauses
        ...
        The HAVING clause is used in a similar manner as the WEHRE clause. 
