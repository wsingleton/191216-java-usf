...
--Create the departments table and define its colums and their constraints
CREATE TABLE departments(
dept_id Number(5) Constraint pk_departments Primary Key,
dept_name VARCHAR2(50) Unique,
monthly_budget NUmber(8, 2)
);
-- create the employees table and define its colums and their constriants
...
CONSTRAINT fk_employee_department Foreign KEY (department_id) references departments(dept_id)
);
...
/*
since we did not add a primary key to the PRODUCTS table, we can alter it to modify the PROD_ID
column and add the PRIMARY KEY constraint to it.
*/
...
-- no need to commit the above statements since all DDL keywords are implicityl comitted

-- add values to the departments table adding insert
...
--DML statments must be explicityl commited
commit;
