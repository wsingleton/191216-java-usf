/*
    ...
    2) An XMLHttpRequestObject is created by JavaScript
    3) The XMLHttpRequest object sends a request to a web server
    4) server processed the request
...
window.onload = function(){
    ...
    //step one if the user click the button
}
...
function getPokemon(){
    ...
    //create connection (endpoint/url, verb, boolean asynch or not)
    xhttp.open("GET", 'https://pokeapi.co/api/v2/pokemon/' + pokemonId, true);
    xhttp.send(); //step three
    xhttp.onreadystatechange = function(){
        /*
            The readystate property hold the status of the 
...
function setValues(pokemon){
    //step six and seven JS not only reads the data but also
    //utilizing DOM manupyulatiion, we are able to render the data
    //onto the web page
