-- THis allows the DBMS to print message to the console (OFF by default)
SET SERVEROUTPUT ON;

-- Function: Get the max ID from the artist table

-- Method signature (method anme, return type, variables to be returned)
...
--Because functions are shema objects (like tables), they can be dropped
...
-- simple, yet primitive way, to invoke functions that return cursor
SELECT after_1968()
FROM DUAL;
...
-- Use if there is no trigger, b/c with the trigger the the next val is callled twice
-- so it skips an increment
INSERT INTO artist
VALUES (artist_pk_seq.NEXTVAL, 'Blind Guardian');

-- The incrementer is not overwritten with the specified number 999. 
-- the artist is added with the next increment.
INSERT INTO artist
VALUES (999, 'MCR');