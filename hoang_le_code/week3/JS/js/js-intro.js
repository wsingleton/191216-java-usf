...
/* Spread operator
    Spead/rest sysntax allows an iterable such as an array expression or string to be
    expanded in places where zero or more arguments (for function call) or elements 
    (for array lterals )
...
// Combining the spread operator with the destructuring operator
...
/*
    ...
    protypel inheritance 
        ...
        js,s inheritance mechanism is not quite the same as java, which uses a
        class-based inheritance system . Instead , js is know for its use og ptoyotypel inheritance
            each object has a private property which holds a link to another 
            oject, called it " prototype" .
            the prototype object has a prototype of it own and so on ultiol an ojhect is reached that has null as its prototype
            methods can be added to object as properties, and any object has accccc
...
for..of and for..in 
for..of 
    uses an object specific interator and loop over its generated value

for..in
    loop over enumerated property name of an array 
*/
...
for (const proper in myObject){
    console.log('myObject.$(property)= $(myObject)')
}
