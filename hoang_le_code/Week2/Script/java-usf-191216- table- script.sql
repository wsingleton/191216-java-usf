...
*/
-- creat departments table and define its colus and thrie constrain 
CREATE TABLE departments(
    dept_id NUMBER(5) CONSTRAINT pk_deparments PRIMARY KEY,
...
-- no need to comit the above statements , since all DDL key words are implicity committed 
...
--- DML state ment must be explicitly commit
COMMIT;


-- Delete vs Truncate

-- ddl so wil implicitly committed, cant not rollback
...
Select emp_fn||','|| emp_ln as "Contact Person", email
From employees;
