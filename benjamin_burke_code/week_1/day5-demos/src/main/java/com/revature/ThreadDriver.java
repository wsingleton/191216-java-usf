...
//           single thread operation - one thing can only happen at a time eg -> js is single threaded
//             java is multi threading
      ...
      //  e1.run(); this does not create a new theread, it runs the logic of the run() method on the "main" thread
        ...
        /*
        .join() method
            using join(), we tell our thread to wait until other(s) complete their operation
            Once those threads are terminated, the thread we invoked .join() on will "join" and
            complete its operation.

            There are overloaded versions of this method, which allow the developer the ability
