...
public class UserServiceTest {
    ...
    @Before
    public void setup() {
        sut = new UserService(userRepo, acctRepo, transRepo);
    ...
    @Test
    public void testSetJointAccountWithValidUsername() {
        User user = new User("Blake", "Dunn", "buhlakay9", "testers!");
        String username = "buhlakay9";
        ...
        when(mockDriver.app()).thenReturn(mockState);
        when(mockState.getCurrentSession()).thenReturn(mockSession);
        when(mockSession.getSessionAccount()).thenReturn(mockAcct);
        when(userRepo.findUserByUserName(anyString())).thenReturn(_user);
        verify(userRepo, times(0)).updateCompositeKey(mockUser, mockAcct);
        sut.setJointAccount(username);
    }

    @Test (expected = InvalidRequestException.class)
    public void testSetJointAccountWithNullString () {
        String username = null;
        sut.setJointAccount(username);
    }

    @Test (expected = InvalidRequestException.class)
    public void testSetJointAccountWithEmptyString() {
        String username = "";
        sut.setJointAccount(username);
    }
}
