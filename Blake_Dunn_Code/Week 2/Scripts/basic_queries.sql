...
-- Get all inforamtion from the PRODUCTS table
SELECT *
FROM products;
...
-- ASC keyword is not required when sorting in ascending order
SELECT *
FROM employees
ORDER BY birthdate;
...
-- we can use the column positions (indexes) instead of thir names (NOT RECOMMENDED)
SELECT *
FROM employees
ORDER BY 6 DESC, 5;
...
/*
    ...
    retreive values that do not come from a particular table in DB.
*/
...
-- You don't want to use GROUP BY to sort anything
SELECT department_id, COUNT(*)
FROM employees
GROUP BY department_id;
